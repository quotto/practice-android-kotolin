...
class ResultActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        myHand = when(id) {
            R.id.gu -> {
                myHandImaeg.setImageResource(R.drawable.gu)
                gu
            }
            R.id.choki -> {
                myHandImaeg.setImageResource(R.drawable.choki)
                choki
            }
            R.id.pa ->{
                myHandImaeg.setImageResource(R.drawable.pa)
                pa
            }
            else -> gu
        }
    ...
    private fun getHand(): Int {
        ...
        if (gameCount == 1) {
            if (gameResult == 2) {
                // 前回の勝負が1回目でコンピュータが勝った場合、コンピュータは次に出す手を変える
